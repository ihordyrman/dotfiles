...
require("lazy").setup({
  ...
  },
  {
    ...
    event = "InsertEnter",
    config = function()
      -- Disable default Tab mapping since you use Tab for blink.cmp
      vim.g.copilot_no_tab_map = true

      -- Remap accept suggestion to Ctrl+y instead
  ...
  },
  {
    ...
    ft = { "fsharp" },
    init = function()
      -- Disable Ionide's built-in LSP since we use fsautocomplete via nvim-lspconfig
      vim.g["fsharp#lsp_auto_setup"] = 0

      -- FSI (F# Interactive) settings
      vim.g["fsharp#fsi_command"] = "dotnet fsi"
      vim.g["fsharp#fsi_keymap"] = "none" -- we'll define our own below

      -- Show tooltips on cursor hold
      vim.g["fsharp#show_signature_on_cursor_move"] = 1
    end,
    config = function()
      -- Send line/selection to FSI
      vim.keymap.set(
        "n",
        "<leader>fs",
        ":FsiSendLine<CR>",
